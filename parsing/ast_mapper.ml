...
module M = struct
  ...
  let map_structure_item sub {pstr_loc = loc; pstr_desc = desc} =
    ...
    | Pstr_value (vbs) -> value ~loc (List.map (sub.value_binding sub) vbs)
    | Pstr_value_rec (vbs) -> value_rec ~loc (List.map (sub.value_binding sub) vbs)
...
module E = struct
  ...
  let map sub {pexp_loc = loc; pexp_desc = desc; pexp_attributes = attrs} =
    ...
    | Pexp_let_and (vbs, e) ->
        let_and ~loc ~attrs (List.map (sub.value_binding sub) vbs)
          (sub.expr sub e)
    | Pexp_let_rec (vbs, e) ->
        let_rec ~loc ~attrs (List.map (sub.value_binding sub) vbs)
          (sub.expr sub e)
