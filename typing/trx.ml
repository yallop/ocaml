(*
  ...
  Multiple stages impose slight complication. First thing to note:
  save three exceptions below, type checking rules are uniform in
  staging. Generally, the type checker does not even need to know
  which stage expression it is type checking. Therefore, most of the
  type checker is unaffected by staging.
  The three exceptions are: variable reference, brackets and escapes.
  For variable reference, we have to check that it is used at the same stage
  it is bound (save CSP, see below). Therefore, when we enter the variable
  into an environment, we have to record its stage. When we type check a
  variable reference, we have to check it happens at the same stage.
  Brackets and escapes increment/decrement stage.


  Generally, the code (Parsetree) is constructed when the program is run.
  ...
  and establishes the region for the gensym variable.
...
MetaOCaml brackets .<e>. as well as escapes .~e are represented in
the Parsetree as extension nodes Pexp_extension, with extension names
metaocaml.bracket (corr. metaocaml.escape). That is,
.<e>. is just the syntactic sugar for [%metaocaml.bracket e].

The bracket extension node may also be annonated with bracket-related
...
scope of virtual bindings and materializing them does all the job. The
...
After lifting, a CSP value becomes an ordinary expression. Previously,
we tried to omptimize CSP appearing in deeply-nested brackets. Since
stage > 1 are rare, it doesn't seem to be worth it. In short, after we generated
code with CSP, the result is the ordinary code: former CSP's are substituted
out and effectively disappear.
...
For more detail, see section "The final? solution to CSP"
*)
...
let initial_env = Env.initial


(* Attributes *)
(* We use attributes for many things. In N111 and earlier, even
   brackets and escapes in Parsetree
   were attributes (now we use extension nodes).
   Still we use attributes for several things, including temporary
   representation of brackets and escapes in Typedtree
*)
...
(* If the attribute below is present, the expression is non-expansive 
...
(* add attr_nonexpansive *)
let note_nonexpansive : bool -> expression -> expression = fun nonexp exp ->
  {exp with exp_attributes = 
        if nonexp then attr_nonexpansive :: exp.exp_attributes
                  else exp.exp_attributes}

(* The following also used only in typedtree, for representing
   special escape nodes before they are translated away.
   We use physical equality comparison, to speed things up
 *)
let attr_escape : attribute = 
  Ast_helper.Attr.mk ~loc:Location.none 
    (Location.mknoloc "metaocaml.escape") (PStr [])
...
(* Now, it can be 
  metaocaml.functionliteral optionally immediately preceded by
  metaocaml.bracket
  metaocaml.value optionally immediately preceded by
  metaocaml.bracket
  metaocaml.escape
*)
let what_stage_attr : attributes -> stage_attr_elim = fun attrs -> 
  let metaocaml_attr_p = function
    | {attr_name= {txt="metaocaml.functionliteral"}}
    | {attr_name= {txt="metaocaml.value"}}
    | {attr_name= {txt="metaocaml.bracket"}}
    | {attr_name= {txt="metaocaml.escape"}}
    | _ -> false
  in
  let (mattrs,attrs) = List.partition metaocaml_attr_p attrs in
  match mattrs with
  | [] -> Stage0
  | [{attr_name= {txt="metaocaml.bracket"}}] ->
      Bracket (mattrs,attrs)
  | [{attr_name= {txt="metaocaml.bracket"}};
     {attr_name= {txt="metaocaml.functionliteral"}}] ->
      FunBracket (mattrs,attrs)
  | [{attr_name= {txt="metaocaml.bracket"}};
     {attr_name= {txt="metaocaml.value"}}] ->
      ValBracket (mattrs,attrs)
  | [{attr_name= {txt="metaocaml.escape"}} as a] ->
      Escape (a,attrs)
  | {attr_name=l} :: _ ->
...
(* XXX
   For locality check, use val_uid? Which is a field in every value, and
   tells which compilation
  unit it is it (see Env.get_unit_name ()) for the current
   unit. Or, check if the value is in a persistent structure... 
*)
...
let qualify_ctor : 
 ...
 fun lid cdesc ->
  let loc = lid.loc in
  match (cdesc.cstr_tag, Btype.cstr_type_path cdesc) with
...
let qualify_label : Longident.t loc -> label_description -> Longident.t loc =
 fun lid ldesc ->
  ...
  | Tconstr((Path.Pident _ as ty_path), _, _) ->
    begin
      try ignore (Env.find_type ty_path initial_env); lid
      with Not_found ->
        trx_error @@ Location.errorf ~loc
...
(* Make a an escape node in a typedtree, temporarily.
   When type-checking at level > 0, we type check as usual,
   but then post-process the typedtree by trx_translate.
   We need some way to represent brackets and escapes at level > 1,
   before thery are post-processed by trx_translate and removed.
   We use attributres. 
*)
(* In a Typedtree, ~e is represented as a sequence
   ...
   escape properly, we get a warning. Still, begin 0; e end
   is syntactically and type-correct .
*)
let texp_zero = (* TExp node for constant 0 *)
  texp_int 0

let texp_escape : Typedtree.expression -> Env.t -> type_expr -> 
  Typedtree.expression =
  fun exp env ty ->
    mk_texp ~env ~attrs:(attr_escape :: exp.exp_attributes)
...
(* Compiling an array *)
(* We use this function for grouping trx_translate-transformed expressions,
...
(* Separate from vletbindings those that represent CSPs.
   CSPs are detected by the presence of the attr_csp in the Parsetree.exp
   The result is a pair: CSP let-bindings and teh rest of the bindings
*)
let vlet_separate_csp : vletbindings -> (vletbindings * vletbindings) =
  let is_csp  = function
    | (_,Code (_,{pexp_attributes=[{attr_name={txt="metaocaml.csp"}}]})) -> true
    | _ -> false
  in fun l -> List.partition is_csp l

(* Format a CSP binding *)
let vlet_csp_format : Format.formatter -> (string * code_repr) -> unit =
  fun fmt -> function
  | (_,Code (_,{pexp_attributes=[{attr_payload=(PStr 
     [{pstr_desc=Pstr_eval ({pexp_desc= Pexp_ident li},_)}])}]})) -> 
    Format.fprintf fmt "%a at %a@." 
    Pprintast.longident li.txt Location.print_loc li.loc
  | _ -> assert false
...
type csp_handling = CSP_ok | CSP_warn | CSP_error

let close_code_delay_check ?(csp=CSP_error) : 
    code_repr -> closed_code_repr * (unit -> unit) =
 function
  | Code ((Nil,vls),ast) when csp=CSP_ok -> (vlet_bind_all vls ast,fun () -> ())
  | Code ((Nil,vls),ast) -> begin match vlet_separate_csp vls with
    | ([],vls) -> (vlet_bind_all vls ast,fun () -> ())
    | (csps,vls) ->  (vlet_bind_all vls ast,fun () -> 
      Format.kasprintf (if csp=CSP_error then failwith else prerr_endline)
      "The code built at %a has hardly serializable CSPs: %a\n\
      It may be allowed in the future: do tell if you need it."
      Location.print_loc ast.pexp_loc 
      (Format.pp_print_list ~pp_sep:Format.pp_print_space vlet_csp_format)
          csps)
  end
...
let close_code_repr : ?csp:csp_handling -> 
  code_repr -> closed_code_repr = fun ?csp cde ->
  let (ast, check) = close_code_delay_check ?csp cde in
  check (); ast
...
let build_bracket : Location.t -> attributes -> code_repr -> code_repr = 
  fun l attrs exp ->
  code_wrapper 
  (let open Ast_helper in
  fun loc e -> Ast_helper.Exp.extension ~loc ~attrs
      (Location.mknoloc "metaocaml.bracket", (PStr [Str.eval e])))
  l exp
let build_escape : Location.t -> code_repr -> code_repr = 
  code_wrapper @@
  let open Ast_helper in
  fun loc e -> Ast_helper.Exp.extension ~loc 
      (Location.mknoloc "metaocaml.escape", (PStr [Str.eval e]))
...
(* previously I used string_of_float to convert a float to a string
   However, it loses precision during conversion.

 One approach is to use hexadecimal literals, which preserve
 floating point information precisely. But it is rather unintuitive.

 let float : float -> string = fun x ->
  if Float.is_integer x then string_of_float x else
            Printf.sprintf "%h" x

 OTH, float64 accuracy is up to 15 to 16 decimal digits,
 so printing 17 digits maintains the needed precision.

 For reference:
 From Andreas Rossberg, Caml list, Thu, 16 Jun 2022 11:01:32 +0200

 "%.17g" is what we use in the WebAssembly reference interpreter as
 well. Plus, we do some extra work to also preserve -0.0 and
 NaNs. Simplified a bit, it's something like this:
...
let float_constant : float -> Parsetree.expression = fun x ->
  let s = 
      if Float.is_integer x then string_of_float x else
      Printf.sprintf "%.17g" x
  in
  Ast_helper.Exp.constant (Const.float s)

let lift_constant_float : float -> code_repr = fun x -> 
  open_code @@ float_constant x
...
(* Lift the run-time value v into a Parsetree for the code that, when
   ...
   literal or as a CSP.
   If the run-time representation is simple and easily serializable (like
   int), we generate the corresponding constant AST node.
   ...
   If the value looks too complex (difficult to serialize, deserialization
   takes too much time, deserialization will allocate non-negligible
   amount of memory, which we want to avoid as this CSP node may
   appear deep into a nested loop), we essentially let-bind it.
   The let-bound expression is Obj.magic attached to the raw heap
   pointer for the value, masquerading as a Const_string, since
   Const_string accepts arbitrary heap pointer, not just a string
   pointer. The pretty-printer should remove such things (but it is
   OK for running within the top-level).
   The name of the variable standing for CSP is built to easily
   distinguish it.
   We attach the CSP attribute to an expression, for the sake of better
   printing, and for the sake of distinguishing and noting them 
   when running code, especially native. See XXX
...
(* referred to at run-time! If you change interface, check
   all texp_apply mentioning it *)
let dyn_quote : Obj.t -> Longident.t loc -> code_repr =
  fun v li ->
   let csp_attr = attr_csp li in
   let (easy,exp) = match Obj.is_int v with
    | true ->
        (true, Ast_helper.Exp.constant (Const.int (Obj.obj v)))
    | false when Obj.tag v = Obj.double_tag ->
        (true, float_constant (Obj.obj v))
    | false when Obj.tag v = Obj.string_tag ->
        (true, Ast_helper.Exp.constant (Const.string (Obj.obj v)))
    | _   ->           (* general case *)
        (*
        let () =
        ...
        *)
        (* Here, we use Const.string x because it allows x
           to be any heap object, not just string.
           We immediately pass it to Obj.magic, so we never have to
           examine if this value x is really a string *)
        (false, Ast_helper.Exp.constant  (Const.string (Obj.obj v)))
   in
   let exp =
     open_code @@
     Ast_helper.Exp.apply ~attrs:(if easy then [] else [csp_attr]) ~loc:li.loc
     obj_magic_exp [(Nolabel,exp)]
   in
   if easy then exp else
    (* see genlet *)
    let name = gensym ("csp_" ^ Longident.last li.txt) in
    Code ((Nil, [(name,exp)]),
        Ast_helper.Exp.ident (mknoloc (Longident.Lident name)))

(* Build the Typedtree node that lifts the value of the variable with the 
   given path and type.
...
let rec trx_pattern : 
  ...
  | Tpat_construct (_li, cdesc, _args, Some _) ->
        trx_error @@ Location.errorf ~loc:pat.pat_loc
          "Patterns with existentials `%s' \
           within brackets are probably unsound. They are not \
           supported" cdesc.cstr_name;
  | Tpat_construct (li, cdesc, args, None) ->
...
(* Patterns are processed left-to-right. The result is the processed
   ...
   See trx_translate for functions, let, match and try
...
(* ------------------------------------------------------------------------ *)
(* The main function to translate away brackets
  This is the optimized function: it deals only with future stage
  (regardless which future stage it is). See the translation paper.
  In the paper, this function is denoted \lfloor . \rfloor
  The source is the typedtree with one extra extension: escape node,
  distinguished by a special attribute. There are no bracket (or CSP)
  nodes. 
...
Essentially the result of trx_translate should be like
...
OTH, if, in the future, we want to unify with tagless-final, then
invoking the building function for each construct is good.
*)
...
(* Given a type [ty], return [ty code code ... code] (n times code).
   ...
   is code_repr.
   Since now trx_translate is used by the type checker proper, we should
...
(* When we translate typedtree, we replace the nodes
   (with function application, usually) and so have to adjust the types.
   The following funtion does the adjustment: it creates a new type_expr
   that has the same level, scope, id as the old expression,
   but with a new, adjusted type_desc.
   We specifically do not use instance (newgenty ...) because we re-write
   the existing expression and existing type, keeping all the
   type var as they are.
   However, we have to give our type a new id, since it is a cons-hash
   code, sort of, for a type.
*)
let fixup_type : type_expr -> type_desc -> type_expr = fun te td ->
  newty3 ~level:(Types.get_level te) ~scope:(get_scope te) td
...
let rec trx_translate : expression -> expression = fun exp ->
  (*
  ...
  *)
  (* Handle the special escape construct, which is distinguished solely
     by attribute: see texp_escape, which placed that attribute
  *)
  if List.memq attr_escape exp.exp_attributes then
    match exp.exp_desc with
    | Texp_sequence (_,exp) -> exp
    | _ -> assert false   (* corrupted representation of escape *)
  else trx_translate_ exp
...
and trx_case_list_body : Typedtree.pattern -> 
  ...
  let body = 
    texp_array (List.map (fun {c_guard;c_rhs;_} -> 
      texp_tuple [texp_option @@ map_option trx_translate c_guard;
                  trx_translate c_rhs])
      cl) in
  { exp with
    exp_desc = Texp_function{arg_label=Nolabel;
                             ...
                             partial=Total};
    exp_type = fixup_type exp.exp_type 
               (Tarrow (Nolabel,binding_pat.pat_type, body.exp_type, commu_ok))
  }


and trx_translate_ : expression -> expression = fun exp ->
  ...
  | Texp_ident (p,li,vd)  ->
    ...
    if stage = 0 then trx_csp exp p li 
      ...
      *)
      Texp_ident (p,li,{vd with val_type = mk_type_code vd.val_type})
  ...
  | Texp_let (Nonrecursive,[{vb_pat = {pat_desc = Tpat_var (id,name)} as pat;
                     vb_expr = e}],ebody) ->
      let pat = {pat with pat_type = mk_type_code pat.pat_type} in
         ...
         { exp with
           exp_desc = 
            Texp_function{arg_label=Nolabel;
                          param=id;
                          cases=[texp_case pat (trx_translate ebody)];
                          partial=Total};
           exp_type = fixup_type exp.exp_type
               (Tarrow (Nolabel,pat.pat_type, 
                        mk_type_code ebody.exp_type, commu_ok))
         }
       ]
  ...
  | Texp_function {arg_label=l;cases=cl} ->
      begin
      match trx_cl cl (mk_type_code (Btype.newgenvar ())) with
            ...
            [texp_loc exp.exp_loc; 
             ...
             texp_array (List.map (fun {c_guard;c_rhs;_} -> 
                 texp_tuple [texp_option @@ 
                              map_option trx_translate c_guard;
                             trx_translate c_rhs]) 
                 cl)
           ]
      ...
      texp_apply (texp_ident "Trx.build_apply")
        [texp_loc exp.exp_loc; 
         texp_array (List.map (fun (l,e) ->
           texp_tuple [texp_label l;trx_translate e]) lel)]
  ...
  | Texp_tuple el ->
      texp_apply (texp_ident "Trx.build_tuple")
        [texp_loc exp.exp_loc; 
	 texp_array (List.map trx_translate el)]
  ...
  | Texp_construct (li, cdesc, args) ->
      ...
      texp_apply (texp_ident "Trx.build_construct")
        [texp_loc exp.exp_loc; 
         texp_lid lid;
	 texp_array (List.map trx_translate args)]

  | Texp_variant (l,eo) ->              (* polymorphic variant *)
      texp_apply (texp_ident "Trx.build_variant")
        [texp_loc exp.exp_loc; 
         texp_string l;
	 texp_option (map_option trx_translate eo)]
        ...
        [texp_loc exp.exp_loc; 
         texp_array (Array.fold_left (fun acc (ldesc,recd) ->
           ...
           texp_tuple [texp_lid (qualify_label li ldesc);
                       trx_translate e] :: acc) [] fields);
         texp_option (map_option trx_translate extended_expression)]
  ...
  | Texp_ifthenelse (e,et,efo) ->
      texp_apply (texp_ident "Trx.build_ifthenelse")
        [texp_loc exp.exp_loc; 
         trx_translate e;
         trx_translate et;
	 texp_option (map_option trx_translate efo)]

  | Texp_sequence (e1,e2) ->
      texp_apply (texp_ident "Trx.build_sequence")
        [texp_loc exp.exp_loc; 
	 trx_translate e1; trx_translate e2]
  | Texp_while (e1,e2) ->
  ...
  | Texp_for (id, pat, elo, ehi, dir, ebody) ->
      ...
      texp_apply (texp_ident "Trx.build_for") 
        [texp_loc exp.exp_loc;
         ...
         trx_translate ehi;
         texp_bool (dir = Upto);
         let var_typ = mk_type_code (Ctype.instance Predef.type_int) in
  ...
  | Texp_send (e,m) ->
	 ...
	 trx_translate e;
         texp_string (match m with
                        ...
                        | Tmeth_ancestor _ -> 
                            not_supported exp.exp_loc 
                              "Objects (Texp_send, ancestor)")]
  ...
  | Texp_assert e ->
      texp_apply (texp_ident "Trx.build_assert")
        [texp_loc exp.exp_loc; trx_translate e]
  ...
  | Texp_open (_od,_e)   -> not_supported exp.exp_loc "let open general"
       ...
       texp_apply (texp_ident "Trx.build_open")
        [texp_loc exp.exp_loc;
         ...
         exp]      (* exp is the result of trx_translate *)
  ...
  {exp with exp_type = mk_type_code exp.exp_type;
            exp_desc = new_desc}

(* Translate an expression and then add an escape node
   The result type is the same as the original exp
   (translation makes <t> from t, but Escape node makes t from <t>
 *)
let trx_translate_and_esc : expression -> expression = fun exp ->
  {exp with exp_desc = texp_apply (texp_ident "Trx.build_escape")
                       [texp_loc exp.exp_loc; 
                        trx_translate exp]}

(* Translate an expression and then add a bracket node
   If the original expression has type t, the result has the type
   <<t>>
 *)
let trx_translate_and_bra : expression -> attributes -> expression = 
  fun exp attrs ->
  {exp with 
   exp_type = mk_type_code (mk_type_code exp.exp_type);
   exp_desc = texp_apply (texp_ident "Trx.build_bracket")
                       [texp_loc exp.exp_loc;
                        texp_attributes attrs;
                        trx_translate exp]}
     ...
     {defl with expr =
       ...
       fun this exp ->
        ...
        | Pexp_extension ({ txt = ("metaocaml.bracket"); _ },
            ...
            incr stage; this.expr this exp; stage := stage_old 
        | Pexp_extension ({ txt = ("metaocaml.escape"); _ },
                    PStr [{pstr_desc = Pstr_eval(exp,_)}]) ->
            if !stage = 0 then res := false (* going negative: esc to
                                               prev level *)
            else
            let stage_old = !stage in
            decr stage; this.expr this exp; stage := stage_old 
        ...
        | Pexp_newtype _ 
          -> defl.expr this exp
        | _  -> 
            (* application, etc. aren't values at 0 stage *)
            if !stage = 0 then res := false else
            defl.expr this exp   (* they are, if children are *)
    end}
    in 
...
(* Essentially, as if it were let local_genlet = ... in k <local_genlet> *)
...
| Texp_bracket e -> 
  ...
  (trx_translate trx_exp 1 e).exp_desc
