...
let type_class_arg_pattern cl_num val_env met_env l spat =
  ...
  begin match l with
  | Tarr_optional _ -> unify_pat val_env pat (type_option (newvar ()));
  | _ -> ()
  end;
  let (pv, met_env) =
    List.fold_right
...
(* NNN:  Instantiates type scheme to a type *)
  ...
  | Pexp_fun (l, Some default, spat, sexp) ->
      ...
      assert (match l with Parr_optional _ -> true | _ -> false);
      let open Ast_helper in
  ...
  | Pexp_fun (arr, None, spat, sexp_body) -> (* NNN fixing the bug: sexp_body *)
      type_function ?in_function loc sexp.pexp_attributes env ty_expected
        arr [{pc_lhs=spat; pc_guard=None; pc_rhs=sexp_body}]
  ...
  | Pexp_letmodule({pmb_name = name; pmb_expr = smodl; pmb_implicit;
      ...
      let mb = {mb_id = id; mb_name = name; mb_expr = modl; mb_loc = pmb_loc;
                mb_implicit = pmb_implicit; mb_attributes = pmb_attributes} in
      re {
...
and type_function ?in_function loc attrs env ty_expected parr caselist =
  ...
  in
  let ty_arg, is_optional = match arr with
    | Tarr_optional _ ->
      let tv = newvar() in
      begin
  ...
  let not_function ty =
    ...
    ls = [] && not tvar
  in
  if is_optional && not_function ty_res then
...
and type_application env funct
  ...
  (* The main job of typing arguments is done by [type_args], see below.
     ...
     directly match the type of the function but might still be valid by, e.g.,
     extending the type of the function.

    typed: list of arguments already typechecked
  ...
  *)
        ...
        let (ty1, ty2) =
          ...
          | td ->
              ...
              match ty_res.desc with
                Tarrow _ ->
                  if (!Clflags.classic || not (has_label arr1 ty_fun)) then
                    raise (Error(sarg1.pexp_loc, env,
                                 Apply_wrong_label(app1, ty_res)))
                  else
  ...
  (* Match arguments (with apply_flag) and arrow types (with arrow_flag) *)
  let rec type_args typed omitted ty_fun ty_fun0 ty_old sargs more_sargs =
    match expand_head env ty_fun, expand_head env ty_fun0 with
      {desc=Tarrow (arr, ty, ty_fun, com); level=lv} as ty_fun',
